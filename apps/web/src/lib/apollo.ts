...
const httpLink = new HttpLink({
  uri: process.env.API_ENDPOINT || process.env.NEXT_PUBLIC_API_ENDPOINT,
})
...
export function initializeApollo(initialState?: InMemoryCache) {
  ...
  if (initialState) {
    ...
    const data = merge<InMemoryCache>(existingCache, initialState, {
...
export function useApollo<
  T extends Record<string, unknown> & {
    __APOLLO_STATE__: InMemoryCache
  },
>(pageProps: T) {
  const state = pageProps[APOLLO_STATE_PROP_NAME]
  const storeRef = useRef<ApolloClient<InMemoryCache>>()
  if (!storeRef.current) {
